...
describe RaffleService do
  ...
  describe '#call' do
    context "when has more then two members" do
      ...
      it "a member doesn't get yourself" do
        @results.each do |r|
          expect(r.first).not_to eq(r.last)
        end
      end

      it "a member x doen't get a member y that get the member x" do
        # @results.each do |r|
        #   expect(@results.find{ |x| x.first == r.last && x.last == r.first }).to be_nil
        # end
      end

    end

    context "when doesn't have more then two members" do
      before(:each) do
        create(:member, campaign: @campaign)
        @campaign.reload
